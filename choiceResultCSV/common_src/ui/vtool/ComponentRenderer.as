package ui.vtool {
	...
	public class ComponentRenderer extends VRenderer {
		...
		private const
			...
			text:VText = new VText(VText.MIDDLE),
		...
		public function ComponentRenderer() {
			addStretch(bg);
			add(text.setBaseFormat(10, 0x591100, 'Myriad Pro'), { left:4, w:56, hP:100 });
		...
		override public function setData(data:Object):void {
			...
			if (Boolean(infoText) != ((item.mode & VOComponentItem.INFO) != 0)) {
				if (infoText) {
					...
					infoText = new VText(VText.CONTAIN).setBaseFormat(10, 0x591100, 'Myriad Pro');
				}
			}
			if (infoText) {
			...
			if (inputText) {
				if (isDigitCache != isDigit) {
					...
					inputText.restrict = isDigit ? new RegExp(VInputText.INT) : null;
					if (isDigit) {
				...
				inputText.maxChars = isDigit ? 5 : 0;
			...
			if (Boolean(okBt) != ((item.mode & VOComponentItem.BUTTON) != 0)) {
				if (okBt) {
					...
					okBt = VToolPanel.createTextButton('ok', 36);
					okBt.layoutH = -100;
