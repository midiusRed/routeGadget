package ui.vtool {
	...
	public class ComponentPanel extends VComponent {
		...
		private const
			grid:VGrid = new VGrid(1, 7, ComponentRenderer, 100, 18, 0, 2, VGrid.H_STRETCH | VGrid.FLOAT_INDEX),
			infoText:VText = new VText().setBaseFormat(12, 0x591100, 'Myriad Pro')
			;
		private var
		...
		public function ComponentPanel() {
			var copyBt:VButton = VToolPanel.createTextButton('lcp', 38, onCopy);
			copyBt.layoutH = 18;
			var style:String = '<p color="0x591100" fontFamily="Myriad Pro" fontSize="12">';
			addChild(new VBox(new <VComponent>[
				regionCb = VToolPanel.createCheckbox(style + 'Rect</p>', true, onRegion),
				layoutCb = VToolPanel.createCheckbox(style + 'Edit</p>', false, onLayoutPanel),
				VToolPanel.createCheckbox(style + '10</p>', ComponentRenderer.digitBtValue == 10, onChangeDigitBtValue),
			copyBt], 6));
