package ui.vbase {
	...
	public class VComponent extends Sprite {
		public static const
			HIDDEN_CHILD:uint = 16777216, //1 << 24, размеры дочернего компонента не влияют на родителя
			EMPTY:int = int.MAX_VALUE,
			V_TOOL_SOLID:uint = 2147483648, //1 << 31, VTool не лезет внутрь компонента
			V_TOOL_HIDDEN:uint = 1073741824 //1 << 30
			;
		...
		public function setMode(value:uint, isSetBit:Boolean = true):void {
			if (isSetBit) {
				mode |= value;
			} else {
				mode &= ~value;
		 ...
		 * @param	useCapture		Фаза захвата
		 */
		public function removeListener(type:String, handler:Function, dispatcher:EventDispatcher = null, useCapture:Boolean = false):void {
			if (!listenerList) {
				return;
			}
			if (!dispatcher) {
		...
		public function changeListener(flag:Boolean, type:String, handler:Function, dispatcher:EventDispatcher = null):void {
			if (hasListener(type, handler, dispatcher) != flag) {
				if (flag) {
					addListener(type, handler, dispatcher);
				} else {
					removeListener(type, handler, dispatcher);
				}
			}
		}
		...
		protected function syncChildLayout(component:VComponent):void {
			...
			if (isContentDependence && (mode & HIDDEN_CHILD) == 0) {
		...
		protected function syncContentSize(invalidContentSize:Boolean):void {
			...
			if (isGeometryPhase) {
				if (isContentDependence || isParentDependence) {
					updateW = uint.MAX_VALUE;
					if ((mode & HIDDEN_CHILD) == 0 && parent is VComponent) {
						(parent as VComponent).syncChildLayout(this);
					} else {
						geometryPhase();
					}
		...
		private function resetValidContentSize(component:VComponent):void {
			while (component) {
				if (component.isContentDependence) {
		...
		public function remove(component:VComponent, isDispose:Boolean = true):void {
			if (component && component.parent == this) {
				...
				if ((component.mode & HIDDEN_CHILD) == 0) {
					validContentSize = false;
					if (isGeometryPhase && isContentDependence) {
						if (parent is VComponent) {
							(parent as VComponent).syncChildLayout(this);
						} else {
							geometryPhase();
		...
		protected function calcContentSize():void {
			...
			for (var i:int = numChildren - 1; i >= 0; i--) {
				...
				if (component && (component.mode & HIDDEN_CHILD) == 0) {
		...
		public function changeLayout(data:Object, isReset:Boolean = true):void {
			if (isReset) {
				resetLayout();
			}
			assignLayout(data);
		...
		public function get isContentDependence():Boolean {
			return layoutW == 0 || layoutH == 0;
		}

		public function get isParentDependence():Boolean {
		...
		*/
		...
		public function calcAccurateW():uint {
			if (layoutW < 0 || isLeftRight) { //если ширина зависит от ширина родителя
				if (parent is VComponent) {
					var component:VComponent = parent as VComponent;
					if (component.layoutW > 0 && !component.isLeftRight) { //ширина родителя определена и фиксирована
						return applyRangeW(isLeftRight ? component.layoutW - (left + right) : component.layoutW * (-layoutW / 100));
					}
				}
			}
			if (layoutW > 0) { //в противном случае при заданной ширине вернем ее
				return applyRangeW(layoutW);
			}
			return 0;
		}
