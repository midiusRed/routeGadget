package ui.vbase {
	...
	public class VInputText extends VText {
		public static const
			...
			UINT:String = '^\\d{0,10}$', //32 бита
			INT:String = '^\\-?\\d{0,10}$',
			NUMBER:String = '^\\-?\\d{0,16}\\.?\\d{0,5}$' //53бита под целую часть
			;
		...
		public function setLimit(maxChars:uint, pattern:String = null, flags:String = null):VInputText {
			this.maxChars = maxChars;
			if (pattern) {
				restrict = new RegExp(pattern, flags);
			}
			return this;
		}
		...
		private function onFlowOperationBegin(event:FlowOperationEvent):void {
			if (event.operation is InsertTextOperation) {
				...
				if (maxChars > 0 || restrict) {
					var textToInsert:String = insertOperation.text;
					var text:String = textFlow.getText();
					...
					if (maxChars > 0) {
						...
						if (length1 == maxChars) {
							...
							if (length1 + length2 > maxChars) {
								insertOperation.text = textToInsert = textToInsert.substr(0, maxChars - length1);
							}
						}
					}
					if (restrict) {
						if (delSelOp) {
							text = text.substr(0, delSelOp.absoluteStart) + textToInsert + text.substr(delSelOp.absoluteEnd);
						} else {
							text = text.substr(0, insertOperation.absoluteStart) + textToInsert + text.substr(insertOperation.absoluteEnd);
						}
						if (text.search(restrict) < 0) {
							event.preventDefault();
						}
					}
				}
		...
		private function onFlowOperationEnd(event:FlowOperationEvent):void {
			if (event.operation is PasteOperation) {
				...
				var hasLimit:Boolean = restrict || maxChars > 0;

				if (!hasLimit && (mode & MULTI_LINE) != 0) {
					return;
				}
				...
				if (!hasLimit && pastedText.indexOf("\n") == -1) {
					return;
				}
