package ui.vbase {
	...
	public class VGrid extends VComponent {
		 ...
		 * @param	rendererW				Ширина рендерера
		 * @param	rendererH				Высота рендерера
		 * @param	hGap					Горизонтальный пробел
		 * @param	vGap					Вертикальный пробел
		 * @param	mode					Режимы
		 */
		public function VGrid(hCount:uint, vCount:uint, rendererClassOrFactory:Object, rendererW:int, rendererH:int, hGap:uint = 0, vGap:uint = 0, mode:uint = 0) {
			if (hCount == 0) {
				mode |= CALC_H_COUNT;
				hCount = 1;
			}
			...
			_hCount = hCount;
			_vCount = vCount;
			this.rendererW = rendererW;
			this.rendererH = rendererH;
		...
		public function get isPages():Boolean {
			return dpLength > _maxRenderer;
		}
		...
		protected function updateRendererData():void {
			...
			var renderer:VRenderer = rendererLen > 0 ? renderList[0] : null;
			var childIndex:int = renderer && renderer.parent ? getChildIndex(renderer) : numChildren; //все рендереры будут идти в порядке от 1го
		...
		override protected function customUpdate():void {
			...
			if (isHCalc || isVCalc) {
				...
				changeRendererCount(
					(isHCalc && (layoutW <= 0 || (right != EMPTY && left != EMPTY))) ? Math.floor(w / (rendererW + _hGap)) : _hCount,
					(isVCalc && (layoutH <= 0 || (bottom != EMPTY && top != EMPTY))) ? Math.floor(h / (rendererH + _vGap)) : _vCount,
					null, 0, false
				);
		...
		override public function getToolPropList(out:Array):void {
			out.push(
				...
				new VOComponentItem('rW', VOComponentItem.DIGIT, rendererW),
				new VOComponentItem('rH', VOComponentItem.DIGIT, rendererH)
			);
		}
		...
		override public function updateToolProp(item:VOComponentItem):void {
			switch (item.key) {
				...
				case 'rW':
					rendererW = item.valueInt;
					syncContentSize(true);
					break;

				case 'rH':
					rendererH = item.valueInt;
					syncContentSize(true);
					break;
			}
		}
		...
		public function spliceRenderer(index:uint):void {
			if (_maxRenderer > 0 && index < _maxRenderer - 1) {
				renderList.push(renderList[index]);
