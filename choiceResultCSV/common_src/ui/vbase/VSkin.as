package ui.vbase {
	...
	public class VSkin extends VComponent {
		public static const
			...
			STRETCH_HIDDEN:uint = uint(STRETCH | HIDDEN_CHILD)
			;
		public static var
...
*/
		...
		private function onExternal(event:Event):void {
			if (externalInfo) {
				...
				if ((mode & EXTERNAL_EVENT) != 0) {
					dispatchEvent(new VEvent(VEvent.COMPLETE, externalInfo));
				}
			}
		}
		...
		public function toFrame(index:int):void {
			if (view is MovieClip) {
				(view as MovieClip).gotoAndStop(index);
			}
		}
