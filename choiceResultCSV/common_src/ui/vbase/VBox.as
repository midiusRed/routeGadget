package ui.vbase {
	...
	public class VBox extends VComponent {
		...
		override protected function syncChildLayout(component:VComponent):void {
			if ((component.getMode() & HIDDEN_CHILD) == 0) {
				syncContentSize(true);
			} else {
		...
		override protected function calcContentSize():void {
			...
			if (list.length > 0) {
				var gap:uint = (list.length - 1) * _gap;
				if (isVertical) {
					c_h += gap;
				} else {
					c_w += gap;
		...
		protected function updateHiddenComponents():void {
			for (var i:int = numChildren - 1; i >= 0; i--) {
				var component:VComponent = getChildAt(i) as VComponent;
				if (component && (component.getMode() & HIDDEN_CHILD) != 0) {
					component.geometryPhase();
				}
			}
		}
		...
		private function horizontal():void {
			...
			var sum_w:uint = _gap * (list.length - 1); //суммарная ширина занимаймаемая компонентами
		...
		private function vertical():void {
			...
			var percent_list:Vector.<uint>; //список % данных
			var sum_h:uint = _gap * (list.length - 1);
		...
		override public function getToolPropList(out:Array):void {
			out.push(new VOComponentItem('gap', VOComponentItem.DIGIT, gap));
		}
